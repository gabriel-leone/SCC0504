...
public class OneRowNim extends TwoPlayerGame implements CLUIPlayableGame {
            ...
            while (!validMove) {
                if (computer != null) { // If computer's turn
                    ...
                    try {
                        ...
                        ui.report("Please enter a number between 1 and " + Math.min(MAX_PICKUP, nSticks));
                        continue;
                    }
                }
                validMove = takeSticks(sticks);
                if (!validMove)
                    ui.report("Illegal move. Please try again.");
            }
            changePlayer();
        } // while
    ...
    public static void main(String args[]) {
        ...
        for (int k = 0; k < m; k++) {
            IPlayer computer;
            kb.report("Escolha o tipo de jogador " + (k+1) + " (1 para Bad, 2 para Super): ");
            int playerType = kb.getKeyboardInteger();
            if (playerType == 1) {
                computer = new NimPlayerBad((OneRowNim) game);
            } else {
                computer = new NimPlayerSuper((OneRowNim) game);
            }
...
class NimPlayerSuper implements IPlayer {
    ...
    public String makeAMove(String prompt) {
        int sticksLeft = game.getSticks();
        int sticksToTake = Math.min(game.MAX_PICKUP, sticksLeft); // take the maximum allowed or the remaining sticks
        return "" + sticksToTake;
    }

    public String toString() {
        String className = this.getClass().toString(); // returns 'class NimPlayerSuper'
        return className.substring(5); // cut off the word 'class'
    }
} // NimPlayerSuper
